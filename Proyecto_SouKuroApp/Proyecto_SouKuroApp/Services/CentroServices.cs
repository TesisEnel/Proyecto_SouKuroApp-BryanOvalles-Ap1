...
namespace Proyecto_SouKuroApp.Services
{
    public class CentroServices
    {
        ...
        {
            return await _contexto.centro_de_Pedidos.AnyAsync(c => c.PedidoId == PedidoId);
        }
        public async Task<Centro_de_Pedidos?> GetCentroPed(int id)
        {
            return await _contexto.centro_de_Pedidos.Include(c => c.Detalle).FirstOrDefaultAsync(c => c.PedidoId == id);
        }
        ...
        {
            var c = await _contexto.centro_de_Pedidos.FindAsync(pedido.PedidoId);
        ...
        public async Task<Centro_de_Pedidos?> Buscar(int PedidoId)
        {
